...
// Per-product ball colours, generated by theme/hero-colors.js.
layers.push('assets/css/hero-colors.css');
...
// Puts the active slide's image name on the body, so the ball colours in
// hero-colors.css follow whatever product is on the card. Watching the slide
// classes rather than hooking the Swiper instance keeps this independent of
// how the template initialises its sliders.
const HERO_SYNC = `
    <script>
    (function () {
      ...
      if (!deck) return;
      function sync() {
        var img = deck.querySelector('.swiper-slide-active .hero-img img');
        if (!img) return;
        var key = (img.getAttribute('src') || '').split('/').pop().replace(/\\.[a-z0-9]+$/i, '');
        if (document.body.getAttribute('data-hero') !== key) document.body.setAttribute('data-hero', key);
      }
      sync();
      new MutationObserver(sync).observe(deck, { subtree: true, attributes: true, attributeFilter: ['class'] });
    })();
    <\/script>`;
html = html.replace('</body>', HERO_SYNC + '\n</body>');
