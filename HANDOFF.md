...
## «محتوای کالا یافت نمی‌شود» — the product pages were mute to machines

ترب, four days after the retired-address fix, with a screenshot of
`/products/golden-goose`:

> با بررسی مجدد به اطلاع می‌رساند لینک‌های ارسالی شما همچنان فاقد محصول
> می‌باشند و محتوای کالا در آن‌ها یافت نمی‌شود. این موارد مربوط به زیرساخت سایت
> شماست و می‌بایست توسط طراح سایت و یا نیروی فنی سایت خودتان بررسی و رفع شود.

The first instinct was that they had simply re-checked the same sample URL and
read «این محصول دیگر عرضه نمی‌شود». That was half of it. **The other half was
true of every product page on the shop and had been from the beginning.**

### What was measured, and how

Nothing in this container reaches vikyplus.ir, so this was asked from a GitHub
runner — a temporary workflow on the working branch, the tool CLAUDE.md
records for any question whose answer is only on production. **The network
path to Iran has recovered**: it answered in thirty seconds, where the probe
job had been returning `http_code 000` on all fifteen paths since 07 Sept.

Against a live, in-stock, entirely ordinary product page:

| field | what the live page sent |
| --- | --- |
| `<title>` | `VikyPlus` |
| `<meta name="description">` | the site-wide sentence, identical on every page |
| `og:title` / `og:*` | none |
| `<link rel="canonical">` | none |
| JSON-LD | **0 blocks** |
| `itemprop` | **0** |

The shoe's name, its price, its sizes and its stock were all on that page —
**as words for a person, and in not one of the fields a program is meant to
read.** A crawler asking «which product is this, what does it cost, is it in
stock» got nothing from any of the places it looks. «محتوای کالا در آن‌ها یافت
نمی‌شود» is a literally accurate description of that page.

**No check here could have failed on it.** Every guard in this repository
renders a page and counts pixels — `check-parity.js`, `check-overflow.js`,
`check-css-subset.js` — and all three answer a question about what a *person*
sees. `ProductPageIsLegibleToAMachineTest` asks the other question, by reading
the fields out of the response and asserting their values.

### What was built

**`App\Support\Seo\PageFacts` is the one place a page's facts are built**, so
the title, the description, the Open Graph tags and the JSON-LD cannot come to
disagree — three copies of one fact is how one of them goes stale. It also
means the `seo_title`, `seo_description` and `canonical_url` columns, which
have been on `products` since the first migration and which **nothing had ever
read**, are finally the override they were put there to be.

The JSON-LD is an `AggregateOffer` and not an `Offer`: a shoe here is several
sizes, each its own `branch_offers` row, and they are not always the same
price — a single `Offer` would pick one and be wrong about the rest.
Availability is `isSellable()`, the same question the basket asks, because a
page claiming InStock for something the checkout then refuses is worse than one
that says nothing.

**Amounts are Rial and are declared `IRR`.** `TorobFeedController` divides by
ten because ترب's own schema asks for Toman; schema.org wants a currency code
beside the number. Same money, two schemas, and both say so where they are
written. `test_the_price_is_the_rial_the_shop_stores` pins it, because being
wrong here is silent and is a factor of ten in public.

**Where the hooks live matters.** `partials/head.blade.php` is generated by
`theme/make-blade.js`, and a hand correction in it is deleted the next time
that script runs — the file says so itself. So `@yield('meta')` and the
per-page description sit in `layouts/storefront.blade.php`, which is
hand-owned. The description is **swapped into the tag that is already there**,
once, rather than added beside it: a second `<meta name="description">` does
not replace the first, it sits under it, and a crawler reading the first learns
nothing new. A page that defines no description renders the head byte for byte
as the generator wrote it.

Measured: `check-parity.js` returns **the same pixel counts with and without
this change** — 22,080 at 1440 and 25,714 at 1920, identical in both runs. The
residual is a local database artefact and not code; the point is that this diff
moves nothing.

### The other half: the shop still sells that shoe

Their screenshot was `/products/golden-goose` showing the «دیگر عرضه نمی‌شود»
panel. The live listing, asked the same morning, has **seven Golden Geese** —
`کتونی گلدن گوس رنگ صورتی Golden Goose` and six more colours. `basalam:import`
makes one product per supplier listing and the supplier lists each colourway
separately, so the retired row is `کتونی گلدن گوس` and the shoe itself is on
the shelf under longer names. Telling somebody who asked for that shoe that it
is gone, while it is behind you, is the wrong answer to give a shopper or an
aggregator.

`ProductController::sameShoeStillOnSale()` is the rule, and it is deliberately
the strict one:

- **Same brand**, always. Without that fence a short retired name could reach
  across the shop, and a wrong redirect is worse than an honest dead end. A
  retired product with no brand gets the gone page.
- **The live title must contain the whole of the retired one**, both folded
  with `fold_persian()` — «the same name with a colour added», which is exactly
  how this supplier names a colourway. This is **not** the fuzzy matching that
  `product_group_id` refuses for this same catalogue: that one has to decide
  two *different* names are one shoe; this one only has to recognise its own
  name inside a longer one. A sandal by the same maker does not match, and
  there is a test that says so.
- **302, not 301.** A permanent redirect claims these two rows are one product
  for ever, which nothing here knows — and it is the one kind of mistake a
  crawler will not let the shop take back. A temporary one says what is true:
  this is the closest thing the shop has *today*.

A retired shoe with no living twin still gets the gone page, and that page now
also says `availability: Discontinued` in the JSON-LD — the same sentence in
the vocabulary the other side of the conversation uses.

### Still open, and it is a real one

**The design gate hides the page from any crawler that does not download
stylesheets.** Measured both here and against the live site: with `*.css`
blocked, a product page comes back `visibility: hidden` with **82 characters**
of text — «سایت در حال به‌روزرسانی است» — where the same page with stylesheets
gives 1,512 characters of shoe. Every page on this site, for any crawler that
skips CSS to save bandwidth.

It is **not** what ترب hit — their screenshotter plainly loads CSS, and the
JSON-LD above is in the raw HTML and needs neither CSS nor JavaScript — but it
is real.

The fix is not a user-agent sniff. On the notice path, append the opaque
overlay **first**, then reveal the document and set `overflow: hidden`: a
person still sees only the white notice (it is `position:fixed; inset:0`,
opaque, at `z-index:99999`, all inline and so independent of any stylesheet),
and a text extractor sees the document under it. Nothing paints before the
overlay is in the DOM, which is the property the gate exists for.

**It is blocked on a landmine rather than on the design.** `theme/make-blade.js`
**cannot run on a clean checkout** — it throws «expected 8 inline scripts after
main.js, found 9» — and the gate lives in the generated
`partials/head.blade.php`, so it cannot be regenerated. Bumping the guard to 9
lets the script run and the head comes out correct, but it also rewrites
`partials/scripts.blade.php`, moving the daily deal's countdown. Checked during
this round: **no script is lost** by that bump — all six inline scripts are
